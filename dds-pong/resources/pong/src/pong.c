...
create_domain_participant(DDS_DomainParticipantFactory *dpf,
		                  DDS_DomainId_t myDomain,
                          char *arg)
{
    ...
    *dpf = DDS_DomainParticipantFactory_get_instance();
    ...
    {
        printf("%s PONG: ERROR - Splice Daemon not running", arg);
        exit(1);
    }
...
static void
create_pong_publisher(DDS_DomainParticipant dp, DDS_Publisher *p, const char* write_partition)
{
...
static void
create_ping_subscriber(DDS_DomainParticipant dp, DDS_Subscriber *s, const char* read_partition)
{
...
int main (int argc, char ** argv)
{
     ...
     * Create participant
     */
    dp = create_domain_participant(&dpf, myDomain, argv[0]);

    /* Create PONG publisher */
    create_pong_publisher(dp, &p, write_partition);
    ...
    create_ping_subscriber(dp, &s, read_partition);
    ...
    create_quit_subscribe_topic(dp, s, PP_quit_topic, &PP_quit_reader, PP_quit_dt);
